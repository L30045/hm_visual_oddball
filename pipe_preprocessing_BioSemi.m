...
locfilepath = {dir([filepath,'sub*']).name};
% EEG = pop_loadxdf([filepath,filename]);
% savepath = filepath;


%% preprocessing
for j = 3:length(locfilepath)
    filename = {dir([locfilepath{j},'\*.xdf']).name};
    savepath = [locfilepath{j},'\'];
    for i = 1:length(filename)
        EEG = pop_loadxdf([filepath,locfilepath{j},'\',filename{i}]);
