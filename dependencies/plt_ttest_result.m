...
merged_lib = ori_merged_lib;
thres_lat = [50, 400; 100, 600];
tmp_struct = struct('diff_gip_std',[],'diff_gip_dev',[],'diff_fix_std',[],'diff_fix_dev',[],...
    'std_epoch',[],'dev_epoch',[],'gip_std',[],'gip_dev',[],'fix_std',[],'fix_dev',[]);
ev_lat_lib = {tmp_struct, tmp_struct};
for cond_i = 1:2
    switch cond_i
        case 1
            fix_add = 0;
    ...
    rm_std_idx = diff_gip_std < thres_lat(cond_i,1) |...
                 ...
                 diff_fix_std < thres_lat(cond_i,1)+fix_add |...
                 diff_fix_std > thres_lat(cond_i,2)+fix_add;
    ...
    rm_dev_idx = diff_gip_dev < thres_lat(cond_i,1) |...
                 ...
                 diff_fix_dev < thres_lat(cond_i,1)+fix_add |...
                 diff_fix_dev > thres_lat(cond_i,2)+fix_add;
    ...
    ev_lat_lib{cond_i}.std_epoch = ~rm_std_idx;
    ev_lat_lib{cond_i}.dev_epoch = ~rm_dev_idx;
    ev_lat_lib{cond_i}.gip_std = ~rm_std_idx(~isnan(diff_gip_std));
    ev_lat_lib{cond_i}.gip_dev = ~rm_dev_idx(~isnan(diff_gip_dev));
    ev_lat_lib{cond_i}.fix_std = ~rm_std_idx(~isnan(diff_fix_std));
    ev_lat_lib{cond_i}.fix_dev = ~rm_dev_idx(~isnan(diff_fix_dev));
    ev_lat_lib{cond_i}.diff_gip_std = diff_gip_std(~rm_std_idx&~isnan(diff_gip_std));
    ev_lat_lib{cond_i}.diff_fix_std = diff_fix_std(~rm_std_idx&~isnan(diff_fix_std));
    ev_lat_lib{cond_i}.diff_gip_dev = diff_gip_dev(~rm_dev_idx&~isnan(diff_gip_dev));
    ev_lat_lib{cond_i}.diff_fix_dev = diff_fix_dev(~rm_dev_idx&~isnan(diff_fix_dev));
end
...
fig = plt_erp(plt_EEG_cir,plt_EEG_tri,tarCh,lock_name,shaded_method,false);
savepath = '/data/projects/yuan/2021 HM_visual_oddball/20230720-ERP_ERPImage/';
% saveas(gcf,[savepath,sprintf('erp_nocorrect_%s_%s_%s.png',cond_name,lock_name, tarCh)])
% close(gcf)
...
%% ERPImage
t_name = {'cir_stim','tri_stim','cir_gip','tri_gip','cir_fix','tri_fix'};
tarCh = 'Cz';
savepath = '/data/projects/yuan/2021 HM_visual_oddball/20230720-ERP_ERPImage/merged_lib/ERPImage/';

for e_i = 1:6
smooth = 10;
var_thres = 0.95;
clim = [-10 10];
plt_EEG = eeg_array(e_i);
...
saveas(gcf,[savepath,sprintf('mergedERPImage_%s_%s.png',t_name{e_i},tarCh)])
close(gcf)
end

%% ERPImage from erpimage
cond_i = 1;
ev_name = 'fix';
pltch = {'Cz','CPz','Pz','Oz','O1','O2'};
savepath = '/data/projects/yuan/2021 HM_visual_oddball/20230720-ERP_ERPImage/merged_lib/ERPImage/';

for i = 1:length(pltch)
tarCh = pltch{i};
smooth = 10;
clim = [-10 10];
switch cond_i
...
switch ev_name
    case 'stim'
        plt_EEG_cir = pop_select(merged_lib{cond_i}.std_epoch,'channel',{tarCh});
        plt_EEG_tri = pop_select(merged_lib{cond_i}.dev_epoch,'channel',{tarCh});
        sort_idx_cir = [];
        sort_idx_tri = [];
    case 'gip'
        plt_EEG_cir = pop_select(merged_lib{cond_i}.gip_std,'channel',{tarCh});
        plt_EEG_tri = pop_select(merged_lib{cond_i}.gip_dev,'channel',{tarCh});
        sort_idx_cir = -ev_lat_lib{cond_i}.diff_gip_std;
        sort_idx_tri = -ev_lat_lib{cond_i}.diff_gip_dev;
    case 'fix'
        plt_EEG_cir = pop_select(merged_lib{cond_i}.fix_std,'channel',{tarCh});
        plt_EEG_tri = pop_select(merged_lib{cond_i}.fix_dev,'channel',{tarCh});
        sort_idx_cir = -ev_lat_lib{cond_i}.diff_fix_std;
        sort_idx_tri = -ev_lat_lib{cond_i}.diff_fix_dev;
end
plt_data_cir = squeeze(plt_EEG_cir.data);
plt_data_tri = squeeze(plt_EEG_tri.data);

figure;
subplot(2,1,1)
erpimage(plt_data_cir,sort_idx_cir,plt_EEG_cir.times,[ev_name,'- Deviant -', cond_name],smooth,0,'caxis',clim);
subplot(2,1,2)
erpimage(plt_data_tri,sort_idx_tri,plt_EEG_tri.times,[ev_name, '- Standard - ',cond_name],smooth,0,'caxis',clim);
saveas(gcf,[savepath,sprintf('%s/mergedERPImage_%s_%s_%s.png',cond_name,cond_name,ev_name,tarCh)])
close(gcf)

end
% figure
% plot(plt_EEG_cir.times, mean(plt_data_cir,2), 'r-')
% hold on
% grid on
% plot(plt_EEG_cir.times, mean(plt_data_tri,2), 'b-')

%% ttest in range [-200, 0], [0, 400],[400, 800], plot ERP mean with stat sig points
...
for cond_i = 1:2
    ...
    for t_i = 1:length(trial_name)
        ...
        preserve_idx = find(ev_lat_lib{cond_i}.(epoch_name{t_i}));
        tmp_epoch = ori_merged_lib{cond_i}.(epoch_name{t_i});
        ...
        for c_i = 1:length(loc_t)-1
            get_idx = intersect(loc_t(c_i)+1:loc_t(c_i+1),preserve_idx);
            loc_tmp = tmp_epoch.data(idx_tarCh,:,get_idx);
...
for p_i = 1:6
    ...
    % stat test
    is_correct = true;
    p = rowfun(@ranksum,table(plt_cir',plt_tri'));
    p = p{:,:}';
    if is_correct
        % ttest in range [-200, 0], [0, 400],[400, 800]
        time_test = [-200, 0; 0, 400; 400, 800]; % ms, time period for statistical test
        plt_p = false(size(p));
        for time_i = 1:size(time_test,1)
            idx_test = plt_t >= time_test(time_i,1) & plt_t<= time_test(time_i,2);
