function vis_gip_loc(epoch_struct, event_time, varargin)
...
plot(x_scaled,y_scaled,'-','linewidth',1,'color',[0,0,1,0.1]);
scatter(x_scaled,y_scaled,'MarkerFaceColor','b','MarkerFaceAlpha',0.1,'MarkerEdgeColor','b','MarkerEdgeAlpha',0.1);


diff_fix_std = event_time.fixStd_time - event_time.std_time;
for i = 1:size(x_scaled,2)
    gip_onset = event_time.diff_gip_std(i);
    fix_onset = diff_fix_std(i);
    if ~isnan(gip_onset)
        [~,plt_onset_gip] = min(abs(plt_t-gip_onset));
        if ~isnan(fix_onset)
            [~,plt_onset_fix] = min(abs(plt_t-fix_onset));
            if fix_onset > gip_onset
                fc = 'g';
                gc = 'y';
            else
                fc = 'm';
...
%             scatter(x_scaled(plt_onset_gip,i),y_scaled(plt_onset_gip,i),'Marker','s',...
%             'MarkerFaceColor',gc,'MarkerFaceAlpha',1,'MarkerEdgeColor',gc,'MarkerEdgeAlpha',1);
%             scatter(x_scaled(plt_onset_fix,i),y_scaled(plt_onset_fix,i),'Marker','d',...
%             'MarkerFaceColor',fc,'MarkerFaceAlpha',1,'MarkerEdgeColor',fc,'MarkerEdgeAlpha',1);
            plot(x_scaled(plt_onset_gip,i),y_scaled(plt_onset_gip,i),'Marker','s','linewidth',5,...
            'Color',gc,'Markersize',10);
            plot(x_scaled(plt_onset_fix,i),y_scaled(plt_onset_fix,i),'Marker','d','linewidth',5,...
