function [epoch_struct, EEG] = epoch_ez_v3(filename, EEG)
...
%       EEG:
%           Preprocessed EEG structure to be added markers and epoched.
...
blue_time = [EEG.event(idx_blue).latency]/EEG.srate*1000; % convert from sample point to time (ms)
...
% find out stimulus onset time
fixStd_time = find_1st_fix([fix_start',fix_end'],gipStd_time,std_time); % fixation time, nan if missing % msec
fixDev_time = find_1st_fix([fix_start',fix_end'],gipDev_time,dev_time); % msec

for i = 1:length(fixStd_time)
    if ~isnan(fixStd_time(i))
        len_event = length(EEG.event);
        EEG.event(len_event+1).type = 'circle_fix_start';
        EEG.event(len_event+1).latency = fixStd_time(i)/1000*EEG.srate; % change to sample point
        EEG.event(len_event+1).urevent = len_event+1;
    else
        len_event = length(EEG.event);
        EEG.event(len_event+1).type = 'circle_fix_missing';
        EEG.event(len_event+1).latency = std_time(i)/1000*EEG.srate; % change to sample point
        EEG.event(len_event+1).urevent = len_event+1;
    end
end

for i = 1:length(fixDev_time)
    if ~isnan(fixDev_time(i))
        len_event = length(EEG.event);
        EEG.event(len_event+1).type = 'triangle_fix_start';
        EEG.event(len_event+1).latency = fixDev_time(i)/1000*EEG.srate; % change to sample point
        EEG.event(len_event+1).urevent = len_event+1;
    else
        len_event = length(EEG.event);
        EEG.event(len_event+1).type = 'triangle_fix_missing';
        EEG.event(len_event+1).latency = dev_time(i)/1000*EEG.srate; % change to sample point
        EEG.event(len_event+1).urevent = len_event+1;
    end
end

t_f_blue = find_near_ev(fix_start,blue_time);
for i = 1:length(t_f_blue)
    if ~isnan(t_f_blue(i))
        ...
        EEG.event(len_event+1).latency = blue_time(i)/1000*EEG.srate; % change to sample point
        EEG.event(len_event+1).urevent = len_event+1;
    end
end
